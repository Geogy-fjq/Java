...
public class GourmetCoffee  {
	...
	public void displayNumberOfOrders(Product product) {
		int sum=0;
		for(Iterator<Order> it=sales.iterator();it.hasNext();) {
			Order order =it.next();
				if(order.getItem(product)!=null) {
					sum+=1;
			    }
		}
		stdOut.println("The number of orders that contain  “"+product.getCode()+"”  is:"+sum);
	}
	...
	public void displayTotalQuantityOfProducts() {
		stdOut.println("The total quantity sold for each product is:"+"\n");
		for(Iterator<Product> it=catalog.iterator();it.hasNext();) {
			Product product=it.next();
			int sum = 0;
			for(Iterator<Order> it1=sales.iterator();it1.hasNext();) {
				Order order=it1.next();
				for(Iterator<OrderItem> it2=order.iterator();it2.hasNext();) {
					OrderItem orderItem=it2.next();
